package com.ordino.domain.warehouse.products.categories.repository;
...
public interface WarehouseProductCategoryRepository extends JpaRepository<WarehouseProductCategory, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(p) > 0 THEN true ELSE false END FROM WarehouseProductCategory c JOIN c.products p WHERE c.id = :id")
